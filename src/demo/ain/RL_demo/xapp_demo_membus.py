...
from ai_logic import HybridAIController, IntentState
...
class AIWrapperAgent:
    def __init__(self, bus: MemBus, server: UnifiedWebServer):
        self.bus = bus
        self.server = server
    ...
    async def run(self):
        ...
        while True:
                        # 1. Process KPIs (Improved Sticky Merge)
            if not q_kpi.empty():
                msg = await q_kpi.get()
                payload = msg.payload.get("kpi", {})
                
                # Merge Cell Metrics: Allow 0, but ignore None
                ...
                for ue in payload.get("UEMetrics", []):
                    uid = ue.get("ue_id")
                    if uid:
                        if uid not in self.all_active_ues: self.all_active_ues[uid] = ue
                        else:
                            for k, v in ue.items():
            ...
            if not q_dev.empty():
                ...
                if self.controller.process_deviation(dev):
                    logger.info(f"[MEMBUS] Transitioning on anomaly: {dev['metric']}")
            ...
            try:
                ...
                for act in actions:
                    ...
                    if a_type == "LLM_REQUEST_PLAN":
                        logger.info(f"[WRAPPER] Offloading LLM reasoning task to background thread...")
                        asyncio.create_task(self._background_anomaly_task())
                    
                    elif a_type == "INTENT_UPDATE":
                        # Prepare payload with the EXACT keys the JS is looking for
                        payload = {
                            "intent_id": uuid.uuid4().hex if act.get('is_manual') else "active_anomaly",
                            "type": act.get('intent_name'),
                            "triggering_intents": act.get('triggering_intents', []), # MUST MATCH JS
                            "procedure": act.get('procedure', []),                   # MUST MATCH JS
                            "scope": self.controller.current_deviation.get('metric', 'Global') if self.controller.current_deviation else "Operator"
                        }
                        
                        # Publish to Membus (DashboardServer uses this)
                        await self.bus.pub("intent.current", make_msg("intent.current", "INTENT", "v1", payload))
                        # Broadcast directly (DashboardClient uses this)
    ...
    async def _background_anomaly_task(self):
        dev = self.controller.current_deviation
        snapshot = dict(self.current_metrics)
        loop = asyncio.get_running_loop()
        
        # Run heavy GPU math in separate thread
        outcome = await loop.run_in_executor(
            self.executor, self.controller.parser.devise_plan, dev['metric'], dev['value'], snapshot
        )
        
        # This calls the print statement inside OTMGenerator
        otm = self.controller.apply_llm_plan(outcome)
        
        # Send OTM to ACTUATOR tab
        await self.server.broadcast("command", {"command": "PUBLISH_OTM", "params": otm})

    async def process_intent(self, text):
        logger.info(f"[API] Manual Intent Received: '{text}'")
        loop = asyncio.get_running_loop()
        
        outcome = await loop.run_in_executor(
            self.executor, self.controller.parser.parse_user_intent, text
        )
        
        if self.controller.apply_manual_intent(outcome, text):
            logger.info(f"[API] Manual Intent processed. Waiting for next Controller Tick to update UI.")
    ...
    def convert_xapp_kpi_to_internal(xapp_kpi, meid):
        ...
        for ue in kpi_data.get("ues", []):
            ...
            for m in ue.get("measurements", []):
                ...
                # --- CHANGE 1: Added "thp" to catch "drb_uethpdl_ueid" ---
                elif "throughput" in n or "thp" in n: 
                    m_dict["UE_DRB_UEThpDl_UEID"] = v * 1e6
        ...
        for m in kpi_data.get("measurements", []):
            ...
            # --- CHANGE 2: Added "thp" to catch cell-level throughput names ---
            elif "throughput" in n or "thp" in n:
                cell_metrics["thr_dl_bps"] = v * 1e6
    ...
    async def _handle(self, reader, writer):
        try:
            while True:
                ...
                if not head: break
                body = await reader.read(struct.unpack("!I", head)[0])
...
async def run_ai_loop_with_membus(tcp_server, target_metric, args):
    ...
    ai_wrapper = AIWrapperAgent(bus, web_server)
    web_server.ai_agent = ai_wrapper

    # RESTORED: Parallel Minirocket Sensors
    minirocket_agents = []
    ...
    async def heartbeat():
        while True:
            sys.stdout.write("."); sys.stdout.flush(); await asyncio.sleep(0.1)

    tasks = [asyncio.create_task(tcp_server.start()), asyncio.create_task(web_server.start()), 
             asyncio.create_task(ai_wrapper.run()), asyncio.create_task(heartbeat())]
...
async def main():
    ...
    # Ignored legacy args for script compatibility
